...
public sealed class SelfServiceJourneyApplicationTests
{
    ...
    [Fact]
    public async Task RearrangementOptions_AvailabilityThrows_BubblesWorkflowException()
    {
        var hold = BookingHold.Rehydrate(
            "booking-1",
            "slot-1",
        ...
        var availability = new Mock<IAvailabilityService>();
        availability.Setup(x => x.HandleAsync(It.IsAny<GetAvailabilityQuery>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new InvalidOperationException("Availability dependency failed."));
        ...
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => service.HandleAsync(
            new GetRearrangementOptionsCommand
            {
                BookingId = "booking-1",
                Limit = 5
        ...
        Assert.Equal("Availability dependency failed.", exception.Message);
    }
}
