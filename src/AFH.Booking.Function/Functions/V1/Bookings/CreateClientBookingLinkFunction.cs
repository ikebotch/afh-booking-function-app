...
public sealed class CreateClientBookingLinkFunction
{
    private readonly IBookingChangeAccessService _accessService;
    private readonly IUnitOfWork _uow;

    public CreateClientBookingLinkFunction(
    ...
    [Function("Bookings_CreateClientLink")]
    public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "v1/bookings/{bookingId}/client-link")]
        HttpRequestData req,
        string bookingId,
        CancellationToken ct)
    {
        var body = await req.ReadJsonAsync<CreateBookingAccessLinkRequest>(ct) ?? new CreateBookingAccessLinkRequest();
        var result = await _accessService.CreateClientLinkAsync(new AppCreateBookingAccessLinkRequest
        {
            BookingId = bookingId,
