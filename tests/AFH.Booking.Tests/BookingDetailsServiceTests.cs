...
public sealed class BookingDetailsServiceTests
{
    ...
    [Fact]
    public async Task HandleAsync_ForCancelledBookingKeepsViewLinkButSuppressesActionLinks()
    {
        var startUtc = new DateTime(2026, 07, 15, 9, 0, 0, DateTimeKind.Utc);
        var hold = BookingHold.Rehydrate(
            "booking-1",
        ...
        Assert.Equal("https://client.example/app/bookings/booking-1?token=client-token", result.Value.ViewBookingUrl);
        Assert.Null(result.Value.CancelBookingUrl);
        Assert.Null(result.Value.RescheduleBookingUrl);
    }
}
